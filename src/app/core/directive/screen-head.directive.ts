...
/**
 * Prototype gate (hub ADR-0042 §2, `wedding-web` T341). Structural directive
 * a screen applies to whichever element (or `<ng-template>`) should render in
 * `private-layout`'s pinned head slot instead of at its own position in the
 * screen's template — e.g. `<header class="header" *appScreenHead>`.
 *
 ...
 * change-detection tree — the CDK Portal shape ADR-0042 flags as the thing
 * to prove before it becomes doctrine.
 */
@Directive({
