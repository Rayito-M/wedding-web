...
neither sort nor uniquely name them. Two letters are also reused — `30-phase-n-…` and
`31-phase-o-…` repeat the letters of `18-` and `19-` — so cite a phase by its **directory name**,
never by its letter alone.
...
| **T344** | `anyComponentStyle` becomes an error | done | `28-phase-x-layout-layer` |
...
| **T350** | `people` stops nesting a scroller inside `main` | done | `28-phase-x-layout-layer` |
...
| **T376** | Settings grows an eighth section: authoring Good to know | todo (reconciliation resolved; waits on `wedding-api` T246 + the row-5 call) | `32-good-to-know-content` |
